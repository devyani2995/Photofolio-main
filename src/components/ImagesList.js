...
function ImagesList({ newimageDetails, setNewimageDetails, currentAlbum, setCurrentAlbum }) {
  // use state for handling the toggle of the form, existence of the carousel and loading spinner
  ...
  // reducer function to add or get or update imgage details 
  ...
  // to add an imgage if not already present to the db and the array
  const addImage = (data) => {
    let add = true;
  ...
  // show the form if not present and set the state with new details for edited imgage
  const editImage = (img) => {
    if (!showImagesForm) {
  ...
  // delete the imgage user want to be deleted
    ...
    <div className="images-list">
      ...
      {isLoading ?
        ...
        imagesDetail.length ?
          ...
          :
          <h2 className="no-image-h2">No images found in the album...</h2>
      }
      {imgCarousel === null ?
        null
        :
